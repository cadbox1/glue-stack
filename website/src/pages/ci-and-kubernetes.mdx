...
staging-->manual(Manual Exploratory Testing);
manual-->merge(Merge Pull Request);
...
- Multi stage builds
  ...
  - Caching multi stage builds
	- Speeds up build time dramatically
	- It's tricky
    	- https://github.com/moby/moby/issues/34715
    	- https://stackoverflow.com/questions/52646303/is-it-possible-to-cache-multi-stage-docker-builds
...
Ingress->>NGINX: http request;
NGINX->>Ingress: static frontend assets;
Ingress->>client: https static frontend assets;
</Mermaid>

API requests
<Mermaid>
sequenceDiagram;
client->>Ingress: https api request;
Ingress->>NGINX: http api request;
NGINX->>API: http api request;
API->>DB: query;
DB->>API: table results;
API->>NGINX: json response;
NGINX->>Ingress: gzip json response;
...
- Github Actions build all services in the monorepo for any change
  - Supports paths they're a [bit](https://help.github.com/en/articles/workflow-syntax-for-github-actions#example-matching-paths-in-nested-directories) [dodgy](https://github.community/t5/GitHub-Actions/GitHub-Actions-workflow-not-triggered-with-path/m-p/30321#M400)
- Going to run out of GCP credits in ~5 months. Hopefully Azure is cheaper!

## Conclusion

- This was a fair bit of work.
	- Started in July and finished in September
	- Only have a few hours a week for my side projects
